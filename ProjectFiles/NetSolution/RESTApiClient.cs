...
public class RESTApiClient : BaseNetLogic
{
    ...
    {
        var timeoutVariable = 10000;       //LogicObject.Get<IUAVariable>("Timeout");
    ...
    {
        var userAgentVariable = "FTOptix";  //LogicObject.Get<IUAVariable>("UserAgent");
    ...
    private HttpRequestMessage BuildGetMessage(Uri url, string userAgent, string bearerToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(bearerToken))
            request.Headers.Authorization = new AuthenticationHeaderValue("Token", bearerToken);
